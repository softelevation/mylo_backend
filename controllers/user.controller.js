...
async function broker_reqest(req, res, next){
	...
	try {
		const user = await jwt.verify(req.headers.authorization, accessTokenSecret);
		let user_id = '-'+user.id+'-';
		const upcoming = await qb.select(['users.name','users.email','users.phone_no','users.image','users.address','users.qualifications','book_nows.status','users.about_me','book_nows.id','book_nows.created_at','book_nows.updated_at']).where({'book_nows.status != ':'completed','book_nows.status != ':'rejected','book_nows.status != ':'cancelled'}).like('book_nows.for_broker',user_id).from('book_nows').join('users','users.id=book_nows.cus_id','left').order_by('book_nows.id','desc').get();
		const completed = await qb.select(['users.name','users.email','users.phone_no','users.image','users.address','users.qualifications','book_nows.status','users.about_me','book_nows.id','book_nows.created_at','book_nows.updated_at']).where({'book_nows.status != ':'pending','book_nows.status != ':'in_progress'}).like('book_nows.for_broker',user_id).from('book_nows').join('users','users.id=book_nows.cus_id').order_by('book_nows.id','desc').get();
